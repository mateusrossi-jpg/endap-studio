...
void main() {
  test('Temporizador TON liga a bobina apos o tempo especificado', () {
    ...
    // Preset de 100ms
    final nodeTON = LadderNode(id: 'N2', type: NodeType.timerTON, config: NodeConfig(presetValue: TagValue.integer(100)));
    ...
    // Scan de 50ms (Ainda nao atingiu 100ms)
    runtime.singleScan(50);
    expect(runtime.tagStore.getBool('T_OUT'), isFalse, reason: 'O tempo ainda nao expirou (50ms)');

    // Mais 50ms (Total 100ms)
    runtime.singleScan(50);
    expect(runtime.tagStore.getBool('T_OUT'), isTrue, reason: 'O temporizador atingiu 100ms, deve ligar a bobina');

    // Desliga a entrada (TON deve resetar)
    runtime.tagStore.setBool('T_IN', false);
    runtime.singleScan(10);
    expect(runtime.tagStore.getBool('T_OUT'), isFalse, reason: 'Ao desligar a entrada o TON reseta e corta energia');
  });
}
